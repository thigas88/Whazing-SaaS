...
export default {
  ...
  watch: {
    notificacaoChatInterno: {
      handler() {
        // console.log('interno', this.notificacaoChatInterno)
        if (this.$router.currentRoute.fullPath.indexOf('atendimento-Interno') < 0 || !this.chatFocado.id || this.chatFocado.id !== this.notificacaoChatInterno.senderId) {
          ...
          if ('Notification' in window && Notification.permission === 'granted') {
            const options = {
              body: `${this.notificacaoChatInterno.text} - ${format(new Date(), 'HH:mm')}`,
              tag: this.notificacaoChatInterno.id,
              renotify: true
            }

            const notification = new Notification(
              `Mensagem de ${this.notificacaoChatInterno.sender.name}`,
              options
            )
