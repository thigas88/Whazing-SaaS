      ...
      >
        <q-toolbar class="q-pr-none menu-header menu-left q-gutter-xs full-width" style="height: 64px">
          ...
          <q-btn flat class="btn-rounded" :class="$q.dark.isActive ? 'text-green bg-black' : ''" :icon="isSilentMode ? 'volume_off' : 'volume_up'" @click="toggleSilentMode" :disable="loadingMount">
            <q-tooltip content-class="text-bold"> {{ isSilentMode ? 'Modo Silencioso Ativado' : 'Modo Silencioso Desativado' }} </q-tooltip>
          </q-btn>
          ...
          <q-btn flat class="btn-rounded" :class="$q.dark.isActive ? 'text-green bg-black' : ''" icon="eva-message-circle-outline" @click="() => $router.push({ name: 'chat-interno' })" :disable="loadingMount">
            ...
            <q-badge v-if="(this.notificacaoInternaNaoLida + (parseInt(notificacoesChat) / 2)) > 0"
              color="red"
              floating
              class="badge-left"
            > {{ this.notificacaoInternaNaoLida + (parseInt(notificacoesChat) / 2) }}</q-badge>
          </q-btn>
          <q-btn flat class="btn-rounded" :class="$q.dark.isActive ? 'text-green bg-black' : ''" icon="mdi-clipboard-list-outline" @click="tarefaEdicao = {}; modalTarefa = true" :disable="loadingMount">
            <q-tooltip content-class="text-bold"> Criar Tarefa </q-tooltip>
                  ...
                  <div v-if="!pesquisaTickets.showAll">
                    ...
                    <q-list dense class="q-my-md">
                      ...
                      <q-item tag="label" v-ripple>
                        ...
                        <q-item-section>
                          <q-item-label>Fechados</q-item-label>
                        </q-item-section>
                      </q-item>
                    </q-list>
        ...
        <q-toolbar class="padding-person menu-left items-center ">
          ...
          <q-btn class="btn-rounded-50" icon="mdi-book-account-outline"
            ...
            <q-tooltip content-class="text-bold"> Carregar Mais Tickets </q-tooltip>
          </q-btn>
          ...
          <div class="full-width q-py-xs">
            ...
            >
              ...
              >
                ...
                <q-tooltip content-class="bg-padrao text-grey-9 text-bold"> Conversas Abertas </q-tooltip>
              </q-tab>
              <q-tab
              ...
              >
                ...
                <q-tooltip content-class="bg-padrao text-grey-9 text-bold"> Conversas Pendentes </q-tooltip>
              </q-tab>
              <q-tab
              ...
              >
                ...
                <q-tooltip content-class="bg-padrao text-grey-9 text-bold"> Conversas Fechadas </q-tooltip>
              </q-tab>
            </q-tabs>
          </div>
        </q-toolbar>
          ...
          <div>
            ...
            <q-card class="bg-white border-radius-none q-mt-sm" style="width: 100%" flat>
              <q-card-section class="text-bold text-center q-pa-sm">
                <q-btn flat class="btn-rounded-50 btn-outline btn-small" icon="mdi-file-pdf-box"
                       :class="$q.dark.isActive ? ('text-green') : ''" @click="downloadPDF">
                ...
                <q-btn v-if="this.userProfile === 'admin'" flat class="btn-rounded-50 btn-outline btn-small" icon="mdi-delete"
                       :class="$q.dark.isActive ? ('text-green') : ''" @click="deleteTicket(ticketFocado.id)">
                  <q-tooltip content-class="bg-primary text-bold">
                    Deletar Ticket
                  </q-tooltip>
                </q-btn>
...
import { ConsultarLogsTicket, ConsultarTickets, DeletarMensagem, DeletarTicket, LocalizarMensagens } from 'src/service/tickets'
  ...
  methods: {
    async downloadPDF() {
      ...
      try {
        ...
        const primeiroContato = mensagens[0]?.contact?.name || 'Contato'
        doc.setFontSize(16)
        doc.text(`Contato: ${primeiroContato} - Ticket: ${this.ticketFocado.id}`, 10, 10)
        let yPosition = 30
    ...
    deleteTicket (ticketId) {
      this.$q.dialog({
        title: 'Atenção!! Deseja realmente deletar o Ticket? ',
        cancel: {
          label: 'Não',
          color: 'primary',
          push: true
        ...
        DeletarTicket(ticketId)
          .then(res => {
            ...
            // Redirecionar para a rota /atendimento/chats
          ...
          .catch(error => {
            console.error(error)
            this.$notificarErro('Não é possível deletar o ticket', error)
          })
        this.loading = false
      })
    },
