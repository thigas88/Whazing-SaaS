      ...
      <div class="row col full-width q-pa-lg">
        <q-card flat bordered class="full-width">
          <q-card-section class="text-h6 text-bold">
            ...
            <div class="q-pa-sm">
              <q-btn flat class="generate-button btn-rounded-50" icon="eva-plus-outline" label="Adicionar Canal"
                @click="modalWhatsapp = true" />
              <q-btn flat class="generate-button btn-rounded-50" icon="mdi-restart" label="Reiniciar Conexões"
                @click="reiniciarConexoes" />
              <q-btn flat class="generate-button btn-rounded-50" icon="mdi-cog" label="Token Hub"
                @click="modalHub = true" />
              <q-btn flat class="generate-button btn-rounded-50" v-if="whatsappNumber" icon="mdi-whatsapp"
                label="Chamar Suporte" @click="abrirWhatsApp" />
      ...
      <div class="container-border q-ma-lg q-pb-md container-rounded-10">
        ...
        <div class="row full-width  q-px-md ">
          <template v-for="item in canais">
            <q-card flat bordered class="col-xs-12 col-sm-5 col-md-4 col-lg-3 " :key="item.id">
              <q-item>
                ...
                <q-item-section side>
                  <q-btn round flat dense icon="eva-edit-outline" @click="handleOpenModalWhatsapp(item)"
                    v-if="isAdmin" />
                  <!-- <q-btn
                round
                flat
              ...
              <q-card-section>
                ...
                <q-select rounded outlined dense label="ChatBot" v-model="item.chatFlowId" :options="listaChatFlow"
                  map-options emit-value option-value="id" option-label="name" clearable
                  @input="handleSaveWhatsApp(item)" />
              </q-card-section>
              <q-card-section>
                <q-select rounded outlined dense label="Fila" v-model="item.queueId" :options="listaFila" map-options
                  emit-value option-value="id" option-label="queue" clearable @input="handleSaveWhatsApp(item)">
                  <q-tooltip>
              ...
              <q-card-section>
                <q-select rounded outlined dense label="Usuário" v-model="item.userId" :options="listaUsuario"
                  map-options emit-value option-value="id" option-label="name" clearable
                  @input="handleSaveWhatsApp(item)">
                  <q-tooltip>
            ...
            <q-checkbox v-if="(item.status == 'DISCONNECTED' || item.status == 'qrcode') && item.type == 'whatsapp'"
              v-model="item.importmessages" label="Importar mensagens ao LER QRCODE"
              @input="handleSaveWhatsApp(item)" />
            <q-card-section>
              <DatePick
                v-if="(item.status == 'DISCONNECTED' || item.status == 'qrcode') && item.type == 'whatsapp' && item.importmessages === true"
                rounded dense hide-bottom-space outlined stack-label bottom-slots label="Data de inicio da importação"
                v-model="item.importOldMessages" @input="handleSaveWhatsApp(item)" />
            </q-card-section>
            <q-card-section>
              <DatePick
                v-if="(item.status == 'DISCONNECTED' || item.status == 'qrcode') && item.type == 'whatsapp' && item.importmessages === true"
                rounded dense hide-bottom-space outlined stack-label bottom-slots label="Data de termino da importação"
                v-model="item.importRecentMessages" @input="handleSaveWhatsApp(item)" />
            </q-card-section>
            <q-checkbox
              v-if="(item.status == 'DISCONNECTED' || item.status == 'qrcode') && item.type == 'whatsapp' && item.importmessages === true"
              v-model="item.importOldMessagesGroups" label="Importar mensagens de GRUPO"
              @input="handleSaveWhatsApp(item)" />
            <q-checkbox
              v-if="(item.status == 'DISCONNECTED' || item.status == 'qrcode') && item.type == 'whatsapp' && item.importmessages === true"
              v-model="item.closedTicketsPostImported" label="Encerrar tickets após importação"
              @input="handleSaveWhatsApp(item)" />
            <q-separator />
            <q-card-actions class="q-gutter-md q-pa-sm q-pt-none" align="center">
              <template v-if="item.type !== 'messenger'">
                <q-btn v-if="item.type == 'whatsapp' && item.status == 'qrcode'" color="primary" label="QR Code"
                  @click="handleOpenQrModal(item, 'btn-qrCode')" icon-right="mdi-qrcode-scan" :disable="!isAdmin"
                ...
                <div v-if="item.status == 'DISCONNECTED'" class="q-gutter-sm">
                  <q-btn v-if="item.type != 'whatsapp'" color="positive" label="Conectar"
                    @click="handleStartWhatsAppSession(item.id)" class="btn-rounded-50 q-mx-sm" />
                  <q-btn v-if="item.status == 'DISCONNECTED' && item.type == 'whatsapp'" color="primary"
                    label="Novo QR Code" @click="handleRequestNewQrCode(item, 'btn-qrCode')"
                    icon-right="mdi-qrcode-scan" :disable="!isAdmin" class="btn-rounded-50 q-mx-sm" />
                ...
                <q-btn
                  ...
                  color="negative" label="Desconectar" icon="eva-wifi-off-outline"
                  @click="handleDisconectWhatsSession(item.id)" :disable="!isAdmin" class="btn-rounded-50 q-mx-sm" />
              </template>

              <q-btn color="negative" icon="eva-trash-outline" @click="deleteWhatsapp(item)" :disable="!isAdmin" dense
                round flat class="absolute-bottom-right">
                <q-tooltip>
                  Deletar conexão
                </q-tooltip>
              </q-btn>
