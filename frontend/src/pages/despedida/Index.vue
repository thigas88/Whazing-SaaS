...
export default {
  ...
  methods: {
    ...
    async listarDespedidasFiltro() {
      this.loading = true
      const response = await ListarDespedidasAll(this.params) // Passa params com userId
    ...
    async listarDespedidas() {
      // Obtém o userId do localStorage ou autenticação
      this.params.userId = localStorage.getItem('userId') // Certifique-se de que o userId esteja correto
      const { data } = await ListarDespedidasAll(this.params) // Passa userId como parâmetro
